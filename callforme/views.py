...
def twilio_call(request):
    ...
    call = client.calls.create(to=phone, from_=settings.TWILIO_DEFAULT_CALLERID,
                               url="http://pickupconnect-staging.djangozoom.net/twiml-response/?user_id=%s" %request.user.id, method='GET')
...
def twiml_response(request):
    ...
    with r.gather(action="http://pickupconnect-staging.djangozoom.net/twiml-connect?contact_id=%s&user_id=%s&" %(contact.id, request.GET['user_id']), timeout=15, method='GET') as g: #finishOnKey=1, 
